...
export function Popup() {
  ...
  const loggedInPopup = () => {
    ...
    return <div id="logged-in-popup"></div>;
  };
  const loggedOutPopup = () => {
    return (
      <div id="logged-out-popup">
        <h1>Log in to continue</h1>
        <p
          style={{
            transition: "0.5s",
            opacity: infoMessageVisible ? "50%" : "0%",
          }}
        >
          <em>Reload this window once logged in</em>
        </p>
        <button
          onClick={() => {
            window.open("https://polybrain.xyz/auth0/login", "_blank");
          }}
        >
          Log In
        </button>
  ...
  return <>{userInfo === null ? loggedOutPopup() : loggedInPopup()}</>;
}
